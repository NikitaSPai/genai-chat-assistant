...
class ChatControllerTest {
    ...
    void shouldReturnChatResponse() throws Exception {
        ...
        ChatResponse response = new ChatResponse(
                "N+1 Problem",
                List.of(
                        "One query fetches the main records.",
                        "Additional queries fetch related records."
                ),
                "SELECT * FROM clubs;"
        );
        ...
        mockMvc.perform(post("/api/v1/chat")
                ...
                .andExpect(jsonPath("$.title")
                        .value("N+1 Problem"))
                .andExpect(jsonPath("$.points[0]")
                        .value("One query fetches the main records."))
                .andExpect(jsonPath("$.points[1]")
                        .value("Additional queries fetch related records."))
                .andExpect(jsonPath("$.code")
                        .value("SELECT * FROM clubs;"));
    }
